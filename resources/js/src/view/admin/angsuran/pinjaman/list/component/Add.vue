<template>
  ...
  <div class="offcanvas offcanvas-end responsive-offcanvas" tabindex="-1" id="offcanvasAddRight"
    ...
    <div class="offcanvas-header">
      <h5 id="offcanvasAddRightLabel">Tambah Angsuran Pinjaman Baru</h5>
            ...
            <div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne"
              ...
              <div class="accordion-body text-sm">
                <div class="input-group mb-3">
                  <div class="form-floating flex-grow-1">
                    <select class="form-select form-select-lg" id="floatingCategory"
                      aria-label="Floating label select example" v-model="model.nasabah.id">
                      <option disabled value="">Pilih Nasabah</option>
                      <option v-for="(row, index) in collections.nasabah" :key="index" :value="row.id">
                        {{ row.nama_lengkap }}
                      </option>
                    </select>
                    <label for="floatingCategory">Nama Nasabah</label>
                  </div>
                  <a class="input-group-text btn mb-0 d-flex vertical-center align-middle"
                    :class="refreshNasabah ? 'btn-success' : 'btn-danger'" v-show="!refreshNasabah"
                    @click.prevent="loadNasabah(model.nasabah.id)" style="z-index: 0;">Load Nasabah</a>
                </div>
                ...
                <div class="form-floating mb-3">
                  ...
                  <label for="floatingLabel">NIK</label>
                </div>

                <div class="row g-2 mb-3">
                  <div class="col-md">
                    <div class="form-floating">
                      <input type="email" disabled class="form-control border-0 bg-transparent shadow-none"
                        id="floatingLabel" placeholder="Kota lahir" v-model="model.nasabah.tempat_lahir">
                      <label for="floatingInputGrid">Tempat Lahir</label>
                    </div>
                  </div>
                  <div class="col-md">
                    <div class="form-floating">
                      <input type="date" disabled class="form-control border-0 bg-transparent shadow-none"
                        id="floatingInputGrid" placeholder="Tanggal Lahir" v-model="model.nasabah.tanggal_lahir">
                ...
                <div class="form-floating mb-3">
                  <input type="text" disabled class="form-control border-0 bg-transparent shadow-none"
                    id="floatingLabel" placeholder="Judul Document" v-model="model.nasabah.no_telepon" />
                  <label for="floatingLabel">Nomor Telfon</label>
                </div>
                <div class="form-floating">
                  <textarea disabled class="form-control border-0 bg-transparent shadow-none"
                    placeholder="Leave a comment here" id="adddoclistfloatingDescription" style="height: 100px"
                    v-model="model.nasabah.alamat"></textarea>
                  <label for="adddoclistfloatingDescription">Alamat</label>
                </div>
                <div class="ms-1 mb-3">
                ...
                <div class="col-md">
                  <div class="form-check form-switch ms-1">
                    <input class="form-check-input d-flex align-items-center" style="height: 20px;" type="checkbox"
                      id="flexSwitchCheckCheckedx" v-model="model.nasabah.flagging" disabled>
                  ...
                  <div class="form">
                    <input class="form-control" id="floatingInputGrid" placeholder="Nominal Flagging"
                      v-model.lazy="model.nasabah.flagging_nominal" v-money="money" disabled>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="accordion-body text-sm">
                ...
                <div class="row g-2 mt-3">
                  <div class="col-md">
                    ...
                    <div class="form-floating mb-3">
                      <input type="text" class="form-control"
                        id="floatingLabel" placeholder="Judul Document" v-model="model.angsuran.norek" />
                      <label for="floatingLabel">Norek</label>
                    </div>
                    <div class="row g-2">
                      <div class="col-md">
                        <div class="form-floating mb-3">
                  ...
                  <div class="col-md">
                    ...
                    <div class="form-floating mb-3">
                      <input type="text" class="form-control"
                        id="floatingLabel" placeholder="Judul Document" v-model="model.angsuran.nomor_pensiunan" />
                      <label for="floatingLabel">Nomor Pensiunan</label>
                    </div>
                    <div class="form-floating mb-3">
                      <input class="form-control" placeholder="Nominal" id="floatingLabel"
                    ...
                    <div class="input-group mb-3">
                      ...
                      <a class="input-group-text btn mb-0 vertical-center"
                        :class="refreshAngsuranBtn ? 'btn-success' : 'btn-danger'" v-show="!refreshAngsuranBtn"
                        @click.prevent="calculateAngsuran" style="z-index: 0;"><i
                          class="fas fa-sync p-2 vertical-center"></i></a>
                    </div>
                  </div>
                </div>

                <div class="form-floating mb-3 text-center mt-3" v-show="!createdSchedule">
                ...
                <div class="border border-1 border-primary rounded-top p-4" v-show="createdSchedule">
...
export default {
  ...
  mounted() {
    emitter.on("AddPinjaman", async (user) => {
      console.log('openAddPinjaman')
      this.user = user;
      ...
      await this.fetchNasabah();
      await this.openModal();
    });
    
    emitter.on("moveToCreate", async (id) => {
      console.log('openAddPinjaman')
      this.id = id;
      this.modalAdd = new bootstrap.Offcanvas("#offcanvasAddRight", {
        backdrop: false,
      ...
      await this.fetchNasabah();
      await this.openModal();
      await this.loadNasabah(id);
      this.refreshNasabah = true;
    });
  },
  methods: {
    asset(path) {
    ...
    async fetchNasabah() {
      let endpoint = `${BASEURL}/api/nasabah/options`;
      let response = await axios.get(endpoint, {
        headers: {
          Authorization: "Bearer " + this.$token(),
        },
      });
      console.log('nasabah', response)
      this.collections.nasabah = response.data.data;
    },
    async loadNasabah(id) {
      console.log(id)
      ...
      try {
        ...
        this.model.nasabah = response.data.data.nasabah
        this.refreshNasabah = true
        Swal.close()
    ...
    async confirmStore() {
      console.log(this.model.angsuran, this.model.nasabah.id)
      if (!this.createdSchedule) {
        Swal.fire("Kesalahan", "Nilai angsuran & Jadwal angsuran belum sesuai.", "error");
        return false;
      }
      if (!this.refreshNasabah) {
        Swal.fire("Kesalahan", "Data nasabah belum sesuai. Silahkan load ulang data nasabah.", "error");
        return false;
      }
    ...
    async store() {
      this.loading = true
      let endpoint = `${BASEURL}/api/angsuran-pinjaman`;
      try {
        Swal.fire({
        ...
        let response = await axios.post(`${endpoint}/${this.model.nasabah.id}`, this.model, {
          headers: {
            Authorization: "Bearer " + this.$token(),
            "Content-Type": "multipart/form-data",
          },
  ...
  watch: {
    ...
    'model.nasabah.id': function () {
      this.refreshNasabah = false
    },
