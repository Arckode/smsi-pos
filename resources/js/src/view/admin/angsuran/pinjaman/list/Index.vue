<template>
    ...
    <add-pinjaman-offcanvas ref="addPinjaman"></add-pinjaman-offcanvas>

    <!-- offcanvas Edit New Document -->
    <!-- <edit-document-offcanvas ref="editDocument"></edit-document-offcanvas> -->

    <!-- offcanvas detail Document -->
    <detail-pinjaman-offcanvas ref="detailPinjaman"></detail-pinjaman-offcanvas>
                ...
                <div class="card border shadow-xs mb-4">
                    <div class="card-header border-bottom pb-0">
                        <div class="d-sm-flex align-items-center">
                            <div>
                                <h6 class="font-weight-semibold text-lg mb-0">Angsuran Pinjaman list</h6>
                                <p class="text-sm">Informasi tentang semua angsuran pinjaman</p>
                            </div>
                            <div class="ms-auto d-flex">
                                <button @click.prevent="addPinjaman"
                    ...
                    <div class="card-body px-0 py-0">
                        ...
                        <div class="table-responsive p-0">
                            ...
                            <h6 class="align-middle text-center text-lg pt-3 text-muted"
                                v-else-if="collection.pinjaman && collection.pinjaman.data.length == 0">Tidak ada data
                            </h6>
                            <table v-else class="table align-items-center mb-0">
                                <thead class="bg-gray-100">
                                    <tr>
                                        <th class="text-sm font-weight-semibold opacity-7">
                                            Kode Angsuran Pinjaman
                                            <br>
                                            Nama Nasabah
                                        </th>
                                        <th class="text-sm font-weight-semibold opacity-7">
                                        ...
                                        <th class="text-sm font-weight-semibold opacity-7">Plafond</th>
                                        <th class="text-center text-sm font-weight-semibold opacity-7">Status
                                            <br>Angsuran Pembayaran
                                        </th>
                                        <!-- <th class="text-center text-xs font-weight-semibold opacity-7">Aksi</th> -->
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="item in collection.pinjaman.data" :key="item.id">
                                        <td class="align-middle text-lg">
                                            <div class="d-flex px-3 py-1">
                                                <div class="d-flex flex-column justify-content-start ms-1">
                                                    <a class="mb-0 text-sm font-weight-semibold text-primary text-bold" @click.prevent="detailPinjaman(item.id)">
                                                        {{ item.kode_pinjaman }}
                                                    </a>
                                            ...
                                            <div class="d-flex px-3 py-1">
                                                <div class="d-flex flex-column justify-content-start ms-1">
                                                    <h6 class="mb-0 text-sm font-weight-semibold">
                                                        {{ new Date(item.tanggal_mulai).toLocaleString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) }}
                                                    </h6>
                                                    <h6 class="mb-0 text-sm font-weight-semibold">
                                                        {{ item.tenor }} Bulan
                                                    </h6>
                                        ...
                                        <td class="align-middle text-lg">
                                            <div class="d-flex px-3 py-1">
                                                <h6 class="mb-0 text-sm font-weight-semibold">
                                                    {{ formatRupiah(item.nilai_plafond) }}
                                                </h6>
                                            </div>
                                        </td>
                        ...
                        <div class="border-top py-3 px-3 d-flex align-items-center flex-wrap">
                            ...
                            <div class="ms-auto">
                                <button v-for="(link, index) in collection.pinjaman.links"
                                    ...
                                    <span v-else-if="index === collection.pinjaman.links.length - 1">
...
<script>
import AddPinjamanOffcanvas from './component/Add.vue';
import EditDocumentOffcanvas from './component/Edit.vue';
import DetailPinjamanOffcanvas from './component/Detail.vue';
...
export default {
    ...
    name: "Admin.Index",
    components: {
        'add-pinjaman-offcanvas': AddPinjamanOffcanvas,
    ...
    async mounted() {
        ...
        await this.fetchPinjamanList();

        // If this page was opened with an editId in query (e.g. from a new tab), open the detail
        const editId = this.$route.query.editId || this.$route.query.moveToDetailEdit;
        if (editId) {
            // open the detail offcanvas / trigger any listeners
            emitter.emit('moveToDetailEdit', editId);
            this.detailPinjaman(editId);

            // remove the query param to avoid re-triggering on refresh
            const newQuery = { ...this.$route.query };
            delete newQuery.editId;
            delete newQuery.moveToDetailEdit;
            this.$router.replace({ query: newQuery }).catch(() => {});
        }

        // If this page was opened with an editId in query (e.g. from a new tab), open the detail
        const nasabahId = this.$route.query.nasabahId || this.$route.query.moveToCreate;
        if (nasabahId) {
            // open the detail offcanvas / trigger any listeners
            emitter.emit('moveToCreate', nasabahId);
            // this.createPinjaman(nasabahId);

            // remove the query param to avoid re-triggering on refresh
            const newQuery = { ...this.$route.query };
            delete newQuery.nasabahId;
            delete newQuery.moveToCreate;
            this.$router.replace({ query: newQuery }).catch(() => {});
        }
    },
    ...
    methods: {
        ...
        async fetchPinjamanList() {
            ...
            try {
                ...
                this.collection.pinjaman = response.data.data
                this.meta.total = response.data.data.total
                this.meta.page.nasabahList.current_page = this.collection.pinjaman.current_page
                this.meta.page.nasabahList.last_page = this.collection.pinjaman.last_page
        ...
        detailPinjaman(id) {
            emitter.emit('detailPinjaman', [id, this.meta.user.role]);
        },
        createPinjaman(id) {
            emitter.emit('createPinjaman', [id, this.meta.user.role]);
        },
        ...
        // },
        addPinjaman() {
            console.log('addPinjaman', this.meta.user)
            emitter.emit('AddPinjaman', this.meta.user);
        },
        getQRCode(item) {
